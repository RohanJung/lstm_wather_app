...
class DataTransformer:
    def __init__(self):
        ...
        self.scaler_target = [MinMaxScaler(), MinMaxScaler(), MinMaxScaler()]

    def transform_data(self, features, targets):
        features_scaled = self.scaler_features.fit_transform(features)
        target_scaled = np.hstack([self.scaler_target[i].fit_transform(targets.iloc[:, i].values.reshape(-1, 1)) for i in range(targets.shape[1])])
        features_reshaped = np.reshape(features_scaled, (features_scaled.shape[0], 1, features_scaled.shape[1]))
        return features_reshaped, target_scaled
